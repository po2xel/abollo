...
sqlite3_statement_backend::load_rowset(int totalRows)
{
    ...
    else
    {
        ...
        for (; i < totalRows && databaseReady_; ++i)
        {
            ...
            if (SQLITE_ROW == res)
            {
                dataCache_[i].resize(numCols);

                for (auto c = 0; c < numCols; ++c)
                {
                    const auto& coldef = columns_[c];
            ...
            {
                const auto zErrMsg = sqlite3_errmsg(session_.conn_);
                std::ostringstream ss;
